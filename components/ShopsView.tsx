...
export const ShopsView: React.FC<ShopsViewProps> = ({
  ...
  const [groupBy, setGroupBy] = React.useState<'area' | 'day'>('area');
  ...
  return (
    <div className="flex flex-col h-full gap-3 animate-fadeIn">
      ...
      <div className="flex bg-slate-100/80 p-1 rounded-xl shadow-inner mb-1 border border-slate-200/60 backdrop-blur-sm">
        <button onClick={() => setGroupBy('area')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[11px] uppercase tracking-wide font-black rounded-lg transition-all duration-300 ${groupBy === 'area' ? 'bg-white text-indigo-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-indigo-600 hover:bg-slate-200/50'}`}>
          <MapPin className="w-3.5 h-3.5" />
          <span>By Area</span>
        </button>
        <button onClick={() => setGroupBy('day')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[11px] uppercase tracking-wide font-black rounded-lg transition-all duration-300 ${groupBy === 'day' ? 'bg-white text-indigo-700 shadow-sm border border-slate-200/50' : 'text-slate-500 hover:text-indigo-600 hover:bg-slate-200/50'}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-3 pt-1 scrollbar-hide snap-x">
        {groupBy === 'area' ? (
          <>
            <button onClick={() => setSelectedAreaId('all')} className={`snap-center px-4 py-2 rounded-full text-[11px] font-bold whitespace-nowrap transition-all border shadow-sm ${selectedAreaId === 'all' ? 'bg-indigo-600 border-indigo-600 text-white shadow-indigo-200/50 scale-105' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:scale-105'}`}>All Areas</button>
            {activeAreas.map(area => (
              <button key={area.id} onClick={() => setSelectedAreaId(area.id)} className={`snap-center px-4 py-2 rounded-full text-[11px] font-bold whitespace-nowrap transition-all border shadow-sm ${selectedAreaId === area.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-indigo-200/50 scale-105' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:scale-105'}`}>{area.name}</button>
            ))}
          </>
        ) : (
          <>
            <button onClick={() => setSelectedDay('all')} className={`snap-center px-4 py-2 rounded-full text-[11px] font-bold whitespace-nowrap transition-all border shadow-sm ${selectedDay === 'all' ? 'bg-emerald-600 border-emerald-600 text-white shadow-emerald-200/50 scale-105' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:scale-105'}`}>All Days</button>
            {WEEKDAYS.map(day => (
              <button key={day} onClick={() => setSelectedDay(day)} className={`snap-center px-4 py-2 rounded-full text-[11px] font-bold whitespace-nowrap transition-all border shadow-sm ${selectedDay === day ? 'bg-emerald-600 border-emerald-600 text-white shadow-emerald-200/50 scale-105' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50 hover:scale-105'}`}>{day}</button>
            ))}
          </>
        )}
      </div>
