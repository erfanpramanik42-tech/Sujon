...
export const NotificationToast: React.FC<NotificationToastProps> = ({ show, title, message, onClose, type = 'error' }) => {
  ...
  return (
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] w-full max-w-sm px-6">
      <div className={`${bgColor} text-white rounded-2xl shadow-xl p-3.5 flex items-center gap-3 border ${borderColor} backdrop-blur-md animate-fadeIn`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className={`${textColor} text-[11px] font-medium leading-tight truncate`}>{message}</div>
        </div>
